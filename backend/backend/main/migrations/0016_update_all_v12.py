...
def read_excel_timetable(apps, schema_editor):
    ...
    len_aud_list = len(audience_list.audience_list)
    log(f"len of audience_list.audience_list = {len_aud_list}", "i")
    for (index, audience) in enumerate(audience_list.audience_list):
        log(f"{index}/{len_aud_list} | {audience.number} {audience.building}", "i")
        ...
        if len(Building.objects.filter(name=building_name)) == 1:
            ...
            if len(Audience.objects.filter(
                    number=audience_number,
                    building__name=building_name)) == 0:
                _audience = Audience.objects.create(
                    number=audience_number,
                    description=f"Аудитория номер: {audience_number} {building_name}",
                    building=build,
...
class Migration(migrations.Migration):
    dependencies = [
        ('main', '0015_add_events_to_timetable_v27'),
    ]
